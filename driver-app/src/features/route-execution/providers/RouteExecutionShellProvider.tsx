...
export function RouteExecutionShellProvider({ children }: PropsWithChildren) {
  ...
  const [store] = useState(() =>
    createRouteExecutionStore(createInitialRouteExecutionStoreState()),
  );
  const [stopDetailTransitionDirection, setStopDetailTransitionDirection] =
    useState<"forward" | "backward">("forward");
  const [routeViewMode, setRouteViewMode] = useState<"route" | "search">(
    "route",
  );
  const [routeSearchQuery, setRouteSearchQuery] = useState("");
  const [assignedRouteScrollTop, setAssignedRouteScrollTop] = useState(0);
  const [lastOpenedStopClientId, setLastOpenedStopClientId] = useState<
    string | null
  >(null);
  ...
  const submitRouteAction = useCallback(
    async (command: DriverRouteActionCommand) =>
      submitRouteActionFlow({
        workspace,
        store,
        command,
        orderStateIds: {
  ...
  const resetAssignedRouteScrollTop = useCallback(() => {
    setAssignedRouteScrollTop(0);
  }, []);

  const resetLastOpenedStopClientId = useCallback(() => {
    setLastOpenedStopClientId(null);
  }, []);

  const prepareRouteStopDetailTransition = useCallback(
    (previousStopClientId: string | null, stopClientId: string) => {
      ...
      const previousIndex = previousStopClientId
        ? (assignedRoute?.stops.findIndex(
            (candidate) => candidate.stopClientId === previousStopClientId,
          ) ?? -1)
        : -1;
      const nextIndex =
        assignedRoute?.stops.findIndex(
      ...
      setStopDetailTransitionDirection(
  ...
  const value = useMemo(
    () => ({
      ...
      resetAssignedRouteScrollTop,
      lastOpenedStopClientId,
      setLastOpenedStopClientId,
      resetLastOpenedStopClientId,
    }),
    [
      assignedRouteScrollTop,
      closeRouteSearch,
  ...
  return (
    <RouteExecutionShellContext.Provider value={value}>
      {children}
    </RouteExecutionShellContext.Provider>
  );
}
