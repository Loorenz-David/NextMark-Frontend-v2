...
export function useAssignedRouteController() {
  ...
  const { store, initializeRouteWorkspace, submitRouteAction } =
  ...
  const navigateToLocation = useCallback(
    async (label: string, address: AssignedRouteViewModel["startLocation"]) => {
      const destination = buildMapNavigationDestinationFromAddress(
        label,
        address,
      );
      if (!destination) {
        showMessage({
          status: "warning",
          message: "This location has no usable destination for navigation.",
        });
        return;
      }
  ...
  const completeRoute = useCallback(async () => {
    ...
    if (payload?.reason === "outside_route_window") {
      showMessage({
        status: 422,
        message:
          "Route completion was ignored because it is outside the route window.",
      });
      return;
    }

    if (
      payload?.reason === "already_recorded" ||
      payload?.reason === "higher_priority_recorded"
    ) {
      return;
    }
  ...
  const pageDisplay = useMemo(
    () =>
      mapAssignedRouteToPageDisplay(
        mergedRouteState.route,
        mergedRouteState.status,
        mergedRouteState.error,
      ),
