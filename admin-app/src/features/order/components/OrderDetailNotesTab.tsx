  ...
  return (
    <div className="admin-glass-panel flex h-[420px] flex-col overflow-hidden rounded-3xl border-border shadow-md!">
      ...
      <div className="flex flex-1 flex-col gap-2.5 overflow-y-auto px-5 py-4.5 scroll-thin">
        {notes.length === 0 ? (
          <div className="rounded-3xl border border-border bg-surface-subtle px-4 py-3 text-sm text-[var(--color-muted)]">
            No notes available.
          </div>
        ) : (
          notes.map((note) => {
            ...
            return (
              <div
                key={note.id}
                className={`rounded-3xl border px-4 py-3 text-sm relative ${style.container}`}
              >
      ...
      <FloatingPopover
        ...
        reference={
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-surface-raised text-[var(--color-muted)] transition-colors hover:bg-surface-sunken hover:text-[var(--color-text)]"
          >
        ...
        <div className="admin-glass-popover w-[180px] rounded-lg border border-[var(--color-border-accent)] p-1 shadow-md">
          {canEditOrderNote(note.type) ? (
            <button
              ...
              className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm text-[var(--color-text)] transition-colors hover:bg-surface-hover"
            >
          ...
          <ConfirmActionButton
            ...
            confirmContent={
              <div className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm text-text">
                <DeleteIcon className="h-4 w-4 text-text" />
