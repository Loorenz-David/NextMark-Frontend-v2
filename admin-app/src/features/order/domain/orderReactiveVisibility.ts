...
const resolveOrderPlanId = (order: Order): number | null => {
  if (
    typeof order.delivery_plan_id === "number" &&
    Number.isFinite(order.delivery_plan_id)
  ) {
...
const normalizePlanIdFilters = (
  value: OrderQueryFilters["plan_id"],
): number[] => {
...
export const reactiveOrderVisibility = (
  ...
  if (filters.order_state != null) {
    ...
    const orderStateName =
      typeof order.order_state_id === "number"
  ...
  if (scheduleFrom !== null || scheduleTo !== null) {
    // Schedule range is backend-authoritative. Local plan metadata may be
    // partial or stale while list rows are already valid from API.
    return true
  }
