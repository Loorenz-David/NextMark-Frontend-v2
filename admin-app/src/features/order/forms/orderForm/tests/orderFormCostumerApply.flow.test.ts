...
export const runOrderFormCostumerApplyFlowTests = () => {
...

  {
    ...
    const result = applySelectedCostumerToOrderForm({
      selectedCostumer: {
        ...
        default_primary_phone: {
          phone: { prefix: "+46", number: "700111222" },
        },
        default_secondary_phone: {
          phone: { prefix: "+46", number: "700333444" },
        },
        default_address: {
          address: {
    ...
    assert(
      result.client_primary_phone.number === "700111222",
      "primary phone should be mapped",
    );
    assert(
      result.client_secondary_phone.number === "700333444",
      "secondary phone should be mapped",
    );
    assert(
      result.client_address?.street_address === "Main 1",
