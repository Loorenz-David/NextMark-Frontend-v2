...
export const cubicMetersToCubicCentimeters = (
  value: number | null | undefined,
) =>
  value === null || value === undefined
    ? null
    : Math.round(value * CUBIC_CENTIMETERS_PER_CUBIC_METER);

export const cubicCentimetersToCubicMeters = (
  value: number | null | undefined,
) =>
  value === null || value === undefined
    ? null
    : roundToDecimals(value / CUBIC_CENTIMETERS_PER_CUBIC_METER, 3);

export const kilogramsToGrams = (value: number | null | undefined) =>
  value === null || value === undefined
    ? null
    : Math.round(value * GRAMS_PER_KILOGRAM);

export const gramsToKilograms = (value: number | null | undefined) =>
  value === null || value === undefined
    ? null
    : roundToDecimals(value / GRAMS_PER_KILOGRAM, 3);

export const minutesToSeconds = (value: number | null | undefined) =>
  value === null || value === undefined
    ? null
    : Math.round(value * SECONDS_PER_MINUTE);

export const secondsToMinutes = (value: number | null | undefined) =>
  value === null || value === undefined
    ? null
    : roundToDecimals(value / SECONDS_PER_MINUTE, 2);
...
const omitUnfilledKeys = <T extends Record<string, unknown>>(
  value: T,
): Partial<T> =>
  Object.fromEntries(
    Object.entries(value).filter(
      ([, fieldValue]) =>
        fieldValue !== null && fieldValue !== undefined && fieldValue !== 0,
    ),
  ) as Partial<T>;
...
const parseCapabilities = (
  value: string,
): ParseResult<VehicleCapability[] | null> => {
  const trimmedValue = value.trim();
  if (!trimmedValue) {
  ...
  if (
    values.some(
      (item) => !vehicleCapabilityValueSet.has(item as VehicleCapability),
    )
  ) {
    return {
      ok: false,
      error: "Capabilities must match the backend enum values.",
    };
  }
...
const parseVehicleEnumValue = <TValue extends string>(
  ...
  if (!allowedValues.has(trimmedValue as TValue)) {
    return {
      ok: false,
      error: `${fieldLabel} must match the backend enum values.`,
    };
  }
...
export const buildVehicleCreateInput = (
  ...
  const fuelTypeResult = parseVehicleEnumValue(
    draft.fuel_type,
    vehicleFuelTypeValueSet,
    "Fuel type",
  );
  ...
  const travelModeResult = parseVehicleEnumValue(
    draft.travel_mode,
    vehicleTravelModeValueSet,
    "Travel mode",
  );
  ...
  const statusResult = parseVehicleEnumValue(
    draft.status,
    vehicleStatusValueSet,
    "Status",
  );
  ...
  const numericFields = {
    max_volume_load_cm3: zeroToNull(
      cubicMetersToCubicCentimeters(toNullableNumber(draft.max_volume_load_m3)),
    ),
    max_weight_load_g: zeroToNull(
      kilogramsToGrams(toNullableNumber(draft.max_weight_load_kg)),
    ),
    ...
    travel_distance_limit_km: zeroToNull(
      toNullableNumber(draft.travel_distance_limit_km),
    ),
    travel_duration_limit_minutes: zeroToNull(
      toNullableNumber(draft.travel_duration_limit_minutes),
    ),
    home_facility_id: toNullableNumber(draft.home_facility_id),
    loading_time_per_stop_seconds: zeroToNull(
      minutesToSeconds(toNullableNumber(draft.loading_time_per_stop_minutes)),
    ),
    unloading_time_per_stop_seconds: zeroToNull(
      minutesToSeconds(toNullableNumber(draft.unloading_time_per_stop_minutes)),
    ),
  ...
  if (Object.values(numericFields).some(Number.isNaN)) {
    return {
      ok: false,
      error: "Vehicle numeric fields must contain valid numbers.",
    };
  }

  const rawPayload: VehicleInput = {
    client_id: clientId,
    ...
    unloading_time_per_stop_seconds:
      numericFields.unloading_time_per_stop_seconds,
    fixed_cost: numericFields.fixed_cost,
  };

  const sanitizedPayload = omitUnfilledKeys(rawPayload) as VehicleInput;
...
export const buildVehicleUpdateFields = (
  ...
  if (
    "registration_number" in draft &&
    draft.registration_number !== undefined
  ) {
  ...
  if ("fuel_type" in draft && draft.fuel_type !== undefined) {
    const result = parseVehicleEnumValue(
      draft.fuel_type,
      vehicleFuelTypeValueSet,
      "Fuel type",
    );
  ...
  const numericFieldMap: Array<
    [keyof VehicleUpdateFields, keyof VehicleFormDraft, string]
  > = [
  ...
  if ("max_volume_load_m3" in draft && draft.max_volume_load_m3 !== undefined) {
    ...
    fields.max_volume_load_cm3 = zeroToNull(
      cubicMetersToCubicCentimeters(numericValue),
    );
  }
  ...
  if (
    "loading_time_per_stop_minutes" in draft &&
    draft.loading_time_per_stop_minutes !== undefined
  ) {
    ...
    fields.loading_time_per_stop_seconds = zeroToNull(
      minutesToSeconds(numericValue),
    );
  }

  if (
    "unloading_time_per_stop_minutes" in draft &&
    draft.unloading_time_per_stop_minutes !== undefined
  ) {
    const numericValue = toNullableNumber(
      draft.unloading_time_per_stop_minutes,
    );
    if (Number.isNaN(numericValue)) {
      return { ok: false, error: "Unloading time must be a valid number." };
    }
    fields.unloading_time_per_stop_seconds = zeroToNull(
      minutesToSeconds(numericValue),
    );
  }
