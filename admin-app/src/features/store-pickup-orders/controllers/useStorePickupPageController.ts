...
import { buildStorePickupPlanSummary } from '../domain/storePickupPlanSummary'
import { useStorePickupPageInitializationFlow } from '../flows/storePickupPageInitialization.flow'

export const useStorePickupPageController = ({
  planId,
  freshAfter,
}: {
  planId: number | null
  ...
  const { plan } = useStorePickupPageInitializationFlow(planId, freshAfter)
  const orders = useOrderStore(useShallow(selectOrdersByPlanId(planId)))

  const summary = useMemo(
    () =>
      buildStorePickupPlanSummary({
        plan,
