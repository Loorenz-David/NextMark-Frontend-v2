...
import { reactiveOrderVisibility } from "@/features/order/domain/orderReactiveVisibility";
import {
  getQueryFilters,
  getQuerySearch,
} from "@/features/order/store/orderQuery.store";
import { useOrderStateStore } from "@/features/order/store/orderState.store";
...
const shouldAddCreatedOrderToVisibleList = (clientId: string) => {
  const search = getQuerySearch().trim();
  if (search.length > 0) {
    return false;
  }

  const order = useOrderStore.getState().byClientId[clientId];
  if (!order) {
    return false;
  }
  ...
  >((acc, [id, stateClientId]) => {
    const numericId = Number(id);
    const state = orderStateStore.byClientId[stateClientId];
  ...
  >((acc, [id, planClientId]) => {
    const numericId = Number(id);
    const routePlan = routePlanStore.byClientId[planClientId];
  ...
  if (
    !reactiveOrderVisibility(order, filters, {
      orderStateNameById,
  ...
  useEffect(() => {
    ...
    const refreshPlanById = async (planId: number) => {
      ...
      const normalized = normalizeEntityMap<DeliveryPlan>(
        payload.route_plan as EntityMap<DeliveryPlan> | DeliveryPlan,
      );
      if (!normalized) {
        return;
      }
    ...
    const handleOrderEvent = (
      ...
      void runDedupedOrderRefresh(orderId, async () => {
        ...
        if (event.event_name === "order.created") {
          const createdClientId = useOrderStore.getState().idIndex[orderId];
          if (
            createdClientId &&
            shouldAddCreatedOrderToVisibleList(createdClientId)
          ) {
            addVisibleOrder(createdClientId);
          }
        }
        ...
        await runDedupedOrderRouteContextRefresh(orderId, () =>
